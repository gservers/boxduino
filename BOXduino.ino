...
void setup() {
  ...
  /*lcd.write(0);
    ...
    lcd.write(4);
    lcd.write(5);*/
  fire.debounceTime = 25;
  fire.multiclickTime = 150;
...
void loop() {
  ...
  if (cnt == 6) serv(baud, res, vbat, duty, rms, mode, puffs, pufftime); break;
  if (cnt == 5) on = false; power(); break;
  if (cnt == 4) inv = !inv; lcd.setInverse(inv); break;
  if (cnt == 3) {
    switch (mode) {
      case 1: mode = 0; break;
      case 0: mode = 1; break;
  ...
  if (cnt == -1) {
    switch (mode) {
      case 0: watt(mosfet, duty, 2); cnt = 0; break;
      case 1: bypass(mosfet, 2); cnt = 0; break;
    } puffs++;
  }
  cnt = 0;
